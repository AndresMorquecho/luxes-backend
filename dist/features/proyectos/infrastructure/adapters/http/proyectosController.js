...
export class ProyectosController {
    ...
    async create(req, res) {
        try {
            ...
            try {
                const createdBy = req.user?.nombre || proyecto.responsable || 'Sistema';
        ...
        try {
            ...
            if (b.fasesAlux !== undefined) {
                try {
                    ...
                    const prevFasesMap = new Map(prevFases.map((f) => [String(f.id), f]));
                    const user = req.user;
            ...
            if (seCompletaInstalacion) {
                ...
                try {
                    await notificarDevolucionHerramientasInstalacion({
                        ...
                        datosInstalacion: datosAGuardar,
                    });
                }
                catch (devolError) {
    ...
    async sincronizarDevoluciones(_req, res) {
        try {
     ...
     */
    async removeArchivoFromBatch(req, res) {
        try {
            const { id, batchId } = req.params;
            ...
            const batch = batches[batchIdx];
            if (Array.isArray(batch.archivos)) {
                batch.archivos = batch.archivos.filter((a) => a.url !== url);
            }
            batches[batchIdx] = batch;
            const datosActualizados = { ...datosExistentes, batches };
            await prisma.proyectoFase.update({
     ...
     */
    async deleteBatchDiseno(req, res) {
        try {
            const { id, batchId } = req.params;
            const faseDisenoExistente = await prisma.proyectoFase.findUnique({
                where: { proyectoId_fase: { proyectoId: String(id), fase: 'DISEÑO' } },
     ...
     */
    async deleteEmptyBatchesDiseno(req, res) {
        try {
            const { id } = req.params;
            const faseDisenoExistente = await prisma.proyectoFase.findUnique({
                where: { proyectoId_fase: { proyectoId: String(id), fase: 'DISEÑO' } },
