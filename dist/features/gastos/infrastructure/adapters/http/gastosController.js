...
async function nextTransferenciaId() {
...
export class GastosController {
    ...
    async list(req, res) {
        try {
            ...
            const origenFiltro = req.query.origen || 'todos';
            const usuarioIdFiltro = req.query.usuarioId || '';
            const metodoPagoIdFiltro = req.query.metodoPagoId || '';
            const startDateFiltro = req.query.startDate || '';
            const endDateFiltro = req.query.endDate || '';
            ...
            let filteredData = [...gastosManual, ...pagosCompra, ...pagosNomina, ...anticiposEmpleados].sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime());
            // Filtering by origen
            ...
            if (usuarioIdFiltro) {
                filteredData = filteredData.filter((g) => g.registradoPor?.id === usuarioIdFiltro);
            }
            ...
            if (search) {
                filteredData = filteredData.filter((g) => g.concepto?.toLowerCase().includes(search) ||
                    g.proveedor?.toLowerCase().includes(search) ||
                    g.referencia?.toLowerCase().includes(search) ||
                    g.notas?.toLowerCase().includes(search));
            }
            // Calculate totals
            const sumMontos = (list) => list.reduce((s, g) => s + Number(g.monto?.toString() || 0), 0);
    ...
    async listMovimientos(req, res) {
        try {
            ...
            if (!tipo || tipo === 'todos' || tipo === 'ingreso') {
                ...
                // Ingresos manuales
                const whereIngresoManual = {
                    fecha: { gte: desdeDate, lte: hastaLimit },
                };
                if (metodoPagoId)
                    whereIngresoManual.metodoPagoId = String(metodoPagoId);
                const ingresosManuales = await prisma.ingreso.findMany({
    ...
    async getDashboardSummary(req, res) {
        ...
        `;
            ...
            // 3. Proformas y Conversión Comercial (Período)
            ...
            for (const prof of proformas) {
                ...
                else if (prof.estado === 'Aprobada' || prof.estado === 'Pagada') {
                    if (prof.estado === 'Aprobada')
                        aprobadasCount++;
                    if (prof.estado === 'Pagada')
                        pagadasCount++;
                    totalVendidoAprobado += total;
                }
            }
            const totalConvertidas = aprobadasCount + pagadasCount;
            const tasaConversion = proformas.length > 0
                ? Number(((totalConvertidas / proformas.length) * 100).toFixed(1))
                : 0;
            // 4. Proyectos, Fases ALUX, Tiempos Límite y Puntualidad
            ...
            // Radar de proyectos activos con su fase y cálculo de tiempo límite
            let fasesVencidasCount = 0;
            let fasesPorVencerCount = 0;
            let fasesEnPlazoCount = 0;
            const proyectosRadar = proyectos
                .filter(p => p.estado === 'ACTIVO')
                .map(p => {
                let faseNombre = p.faseActual;
                let faseIndex = 1;
                let totalFases = 5;
                let fechaLimiteFase = p.fechaEntregaEstimada ? new Date(p.fechaEntregaEstimada) : null;
                if (p.fasesAlux) {
                    try {
                        const parsed = JSON.parse(p.fasesAlux);
                        if (Array.isArray(parsed) && parsed.length > 0) {
                            totalFases = parsed.length;
                            const activeFaseObj = parsed.find((f) => f.id === p.faseActual || f.nombre === p.faseActual || f.estado === 'EN_CURSO') || parsed[0];
                            if (activeFaseObj) {
                                faseNombre = activeFaseObj.nombre || faseNombre;
                                const idx = parsed.findIndex((f) => f.id === activeFaseObj.id);
                                if (idx >= 0)
                                    faseIndex = idx + 1;
                                if (activeFaseObj.fechaLimite) {
                                    fechaLimiteFase = new Date(activeFaseObj.fechaLimite);
                                }
                            }
                        }
                    }
                ...
                let urgencia = 'SIN_FECHA';
                if (fechaLimiteFase && !isNaN(fechaLimiteFase.getTime())) {
                    fechaLimiteFase.setHours(0, 0, 0, 0);
                    const diffTime = fechaLimiteFase.getTime() - today.getTime();
                ...
                return {
                    id: p.id,
                    nombre: p.nombre,
                    clienteNombre: p.clienteNombre || p.clienteEmpresa || 'Cliente ALUX',
                    responsable: p.responsable || 'Sin asignar',
                    faseActual: p.faseActual,
                    faseNombre,
                    faseIndex,
                    totalFases,
                    fechaLimite: fechaLimiteFase ? fechaLimiteFase.toISOString().split('T')[0] : null,
                    diasRestantes,
                    urgencia,
                };
            })
                .sort((a, b) => {
                // Ordenar primero vencidos, luego por vencer, luego por fecha
                const score = (u) => (u === 'VENCIDO' ? 1 : u === 'HOY' ? 2 : u === 'POR_VENCER' ? 3 : u === 'A_TIEMPO' ? 4 : 5);
                if (score(a.urgencia) !== score(b.urgencia))
                    return score(a.urgencia) - score(b.urgencia);
                if (a.diasRestantes !== null && b.diasRestantes !== null)
                    return a.diasRestantes - b.diasRestantes;
                return 0;
            });
            // Cálculo de Puntualidad en Entregas (Proyectos completados)
            ...
            let entregasConRetraso = 0;
            for (const proy of proyectosCompletados) {
                if (!proy.fechaCompletado) {
                    entregasATiempo++;
                    continue;
                }
                if (!proy.fechaEntregaEstimada) {
                    entregasATiempo++;
                    continue;
                }
                const fc = new Date(proy.fechaCompletado);
                const fe = new Date(proy.fechaEntregaEstimada);
                fc.setHours(0, 0, 0, 0);
            ...
            const totalCompletados = proyectosCompletados.length;
            const tasaPuntualidad = totalCompletados > 0
                ? Number(((entregasATiempo / totalCompletados) * 100).toFixed(1))
                : 100;
            // Reclamos y Garantías Post-Venta
            const reclamosPendientesCount = await prisma.reclamoProyecto.count({
                where: { estado: { in: ['PENDIENTE', 'EN_REVISION', 'EN_PROCESO'] } }
            });
            const reclamosFinalizadosCount = await prisma.reclamoProyecto.count({
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    kpi: {
                        ...
                        changeCxPPendientes,
                        tasaConversion,
                        proformasCotizadasMonto: totalCotizado,
                        proformasConvertidasMonto: totalVendidoAprobado,
                        totalConvertidas,
                        tasaPuntualidad,
                        entregasATiempo,
                        entregasConRetraso,
                    ...
                    conversionComercial: {
                        ...
                        totalEmitidas: proformas.length,
                        totalConvertidas,
                        tasaConversion,
                        porAprobar: porAprobarCount,
                        rechazadas: rechazadasCount,
                    ...
                    radarFases: proyectosRadar,
                    fasesAlertas: {
                        vencidas: fasesVencidasCount,
                        porVencer: fasesPorVencerCount,
                        enPlazo: fasesEnPlazoCount,
                    },
                    puntualidad: {
                        totalCompletados,
                        entregasATiempo,
                    ...
                    reclamosPostVenta: {
                        pendientes: reclamosPendientesCount,
                        finalizados: reclamosFinalizadosCount,
                    },
                    usersActivity,
                    currentPrintingJob,
                    printQueue,
                    ...
                    quickSummary: {
                        ...
                        reclamosPendientes: reclamosPendientesCount,
                    }
                }
            });
        }
    ...
    async listIngresos(req, res) {
        try {
            ...
            const where = {};
            if (metodoPagoIdFiltro)
                where.metodoPagoId = metodoPagoIdFiltro;
    ...
    // --- TRANSFERENCIA CRUD ---
    async listTransferencias(req, res) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 25;
            const search = (req.query.search || '').toLowerCase();
            const originId = req.query.origenMetodoId || '';
            const destId = req.query.destinoMetodoId || '';
            const startDateFiltro = req.query.startDate || '';
            const endDateFiltro = req.query.endDate || '';
            const where = {};
            if (originId)
                where.origenMetodoId = originId;
            if (destId)
                where.destinoMetodoId = destId;
            if (startDateFiltro || endDateFiltro) {
                where.fecha = {};
                if (startDateFiltro)
                    where.fecha.gte = new Date(startDateFiltro);
    ...
    async removeTransferencia(req, res) {
        try {
            const { id } = req.params;
