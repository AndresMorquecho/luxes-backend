...
import { parseEcuadorStartDate, parseEcuadorEndDate, getEcuadorDateString, buildEcuadorQueryRange, parseDateOnly } from '../../../../../shared/utils/dateOnly.js';
...
export class GastosController {
    ...
    async previewCierre(req, res) {
        try {
            ...
            const transRecibidasPrevMap = {};
            const transEnviadasPrevMap = {};
    ...
    async testDebug(req, res) {
        try {
            ...
            const trace = nominas.map(n => {
                const ingObj = n.ingresos ? (typeof n.ingresos === 'string' ? JSON.parse(n.ingresos) : n.ingresos) : {};
                const egrObj = n.egresos ? (typeof n.egresos === 'string' ? JSON.parse(n.egresos) : n.egresos) : {};
                const ingVal = Object.values(ingObj).reduce((sum, v) => sum + (typeof v === 'number' ? v : (typeof v === 'string' ? Number(v) || 0 : 0)), 0);
                const egrVal = Object.values(egrObj).reduce((sum, v) => sum + (typeof v === 'number' ? v : (typeof v === 'string' ? Number(v) || 0 : 0)), 0);
    ...
    async getBalancesReport(req, res) {
        try {
            const { desde, hasta } = req.query;
            // 1. Setup date ranges
            let desdeDate;
                ...
                try {
                    ...
                    if (encuesta && encuesta.completada) {
                        ...
                        if (hasDateFilter && fechaSurvey) {
                            if (fechaSurvey < desdeDate || fechaSurvey > hastaLimit)
                                continue;
                        }
            ...
            for (const prof of activeProformas) {
                ...
                if (day <= 7)
                    ventasPorSemana['Semana 1'] += profTotal;
                else if (day <= 14)
                    ventasPorSemana['Semana 2'] += profTotal;
                else if (day <= 21)
                    ventasPorSemana['Semana 3'] += profTotal;
                else if (day <= 28)
                    ventasPorSemana['Semana 4'] += profTotal;
                else
                    ventasPorSemana['Semana 5'] += profTotal;
            }
            ...
            const ctasPorCobrarPorMes = {};
            const ctasPorCobrarDetalle = [];
            ...
            for (const n of nominas) {
                ...
                try {
                    ...
                    const ingExtras = Object.values(ingObj).reduce((sum, v) => sum + (typeof v === 'number' ? v : (typeof v === 'string' ? Number(v) || 0 : 0)), 0);
                    ingVal = salarioBase + ingExtras;
                    egrVal = Object.values(egrObj).reduce((sum, v) => sum + (typeof v === 'number' ? v : (typeof v === 'string' ? Number(v) || 0 : 0)), 0);
                ...
                totalIess += iessVal;
                const role = n.empleado?.nombre || 'Sin nombre';
                ...
                const day = f.getUTCDate();
                if (day <= 7)
                    gastosPorSemana['Semana 1'] += costoLaboral;
                else if (day <= 14)
                    gastosPorSemana['Semana 2'] += costoLaboral;
                else if (day <= 21)
                    gastosPorSemana['Semana 3'] += costoLaboral;
                else if (day <= 28)
                    gastosPorSemana['Semana 4'] += costoLaboral;
                else
                    gastosPorSemana['Semana 5'] += costoLaboral;
            }
            for (const oc of ocs) {
                ...
                const day = f.getUTCDate();
                if (day <= 7)
                    gastosPorSemana['Semana 1'] += Number(oc.total);
                else if (day <= 14)
                    gastosPorSemana['Semana 2'] += Number(oc.total);
                else if (day <= 21)
                    gastosPorSemana['Semana 3'] += Number(oc.total);
                else if (day <= 28)
                    gastosPorSemana['Semana 4'] += Number(oc.total);
                else
                    gastosPorSemana['Semana 5'] += Number(oc.total);
            }
            for (const g of allGastosGeneral) {
                ...
                if (cat === 'vehiculos')
                    targetCat = 'Vehículos';
                else if (cat === 'redes_y_programas')
                    targetCat = 'Redes y Programas';
                else if (cat === 'servicios')
                    targetCat = 'Servicios Básicos';
                else if (cat === 'oficina')
                    targetCat = 'Oficina';
                ...
                const day = f.getUTCDate();
                if (day <= 7)
                    gastosPorSemana['Semana 1'] += Number(g.monto);
                else if (day <= 14)
                    gastosPorSemana['Semana 2'] += Number(g.monto);
                else if (day <= 21)
                    gastosPorSemana['Semana 3'] += Number(g.monto);
                else if (day <= 28)
                    gastosPorSemana['Semana 4'] += Number(g.monto);
                else
                    gastosPorSemana['Semana 5'] += Number(g.monto);
            }
            const abonosCompra = await prisma.abonoCompra.findMany({
                where: hasDateFilter ? { fecha: { gte: desdeDate, lte: hastaLimit } } : undefined
            });
                    ...
                    if (Array.isArray(abArr)) {
                        for (const ab of abArr) {
                            ...
                            if (hasDateFilter && abFecha) {
                                if (abFecha < desdeDate || abFecha > hastaLimit)
                                    continue;
                            }
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    cuentasPorPagar: monthsList.reduce((acc, m) => {
