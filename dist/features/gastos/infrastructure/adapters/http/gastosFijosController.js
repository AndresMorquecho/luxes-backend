...
function calculateNextDueDate(currentDate, frecuencia, diaVencimiento) {
...
export async function checkAndNotifyGastosFijosVencidos() {
    try {
        if (!prisma.gastoFijo)
            return 0;
...
export class GastosFijosController {
    ...
    async list(_req, res) {
        try {
            ...
            const deudasCount = items.filter((i) => i.activo && new Date(i.proximaFechaPago) <= todayDate).length;
    ...
    async create(req, res) {
        try {
            const { nombre, categoria, montoEstimado, frecuencia, diaVencimiento, proximaFechaPago, proveedor, notas, metodoPagoId, } = req.body;
            if (!nombre?.trim()) {
            ...
            const nuevo = await prisma.gastoFijo.create({
                data: {
                    ...
                    registradoPorUserId: req.user?.id || null,
                },
            });
    ...
    async update(req, res) {
        try {
            ...
            const { nombre, categoria, montoEstimado, frecuencia, diaVencimiento, proximaFechaPago, proveedor, notas, activo, metodoPagoId, } = req.body;
    ...
    async pagar(req, res) {
        try {
            const id = String(req.params.id);
            const { monto, metodoPagoId, fecha, notas, proveedor, concepto } = req.body;
            ...
            const nuevaProximaFecha = calculateNextDueDate(gf.proximaFechaPago, gf.frecuencia, gf.diaVencimiento);
