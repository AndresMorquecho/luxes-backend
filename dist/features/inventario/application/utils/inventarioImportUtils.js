...
export const CATEGORIAS_VALIDAS = ['Taller', 'Oficina', 'Impresión'];
...
export function resolveSubtipoId(categoria, raw) {
    return buildSubtipoLookup(categoria).get(normalizeKey(raw)) ?? null;
}
export function findUnidad(unidades, raw) {
    const key = normalizeKey(raw);
    if (!key)
        return null;
    return unidades.find(u => normalizeKey(u.nombre) === key ||
        normalizeKey(u.abreviacion) === key ||
        normalizeKey(`${u.nombre} (${u.abreviacion})`) === key) ?? null;
}
export function validateImportItem(raw, line, categoria, unidades) {
    ...
    if (!subtipoRaw)
        messages.push('subtipo es obligatorio');
    ...
    if (showExtraFields) {
        ...
        if (aCargo && payload.estadoUso !== 'BODEGA')
            payload.aCargo = aCargo;
    }
...
function cellValue(cell) {
    ...
    if (typeof v === 'object' && 'result' in v)
        return Number(v.result) || String(v.result ?? '');
...
export async function parseImportExcelBuffer(buffer, categoria, unidades) {
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        ...
        effectiveHeaders.forEach((key, idx) => {
            if (key)
                raw[key] = cellValue(row.getCell(idx + 1));
        });
        const nombre = String(raw.nombre ?? '').trim();
...
export async function buildImportTemplateBuffer(categoria) {
    const workbook = new ExcelJS.Workbook();
...
export function templateFilename(categoria) {
