...
export function createAsistenciaRoutes(controller) {
    ...
    router.get('/horario-config', authMiddleware, requireRoles(['admin', 'Administrador']), (req, res) => controller.getHorarioConfig(req, res));
    router.put('/horario-config', authMiddleware, requireRoles(['admin', 'Administrador']), (req, res) => controller.saveHorarioConfig(req, res));
