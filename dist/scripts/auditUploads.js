...
async function getAllFilesOnDisk(dirPath) {
    const files = [];
    try {
        ...
        try {
            ...
            if (Array.isArray(abonosArr)) {
                abonosArr.forEach((ab) => {
                    if (ab && typeof ab.comprobanteUrl === 'string')
                        addUrl(ab.comprobanteUrl);
                    if (ab && typeof ab.url === 'string')
                        addUrl(ab.url);
                });
            }
        }
...
export async function runUploadsAudit(cleanOrphans = false) {
    ...
    if (orphanedFiles.length > 0) {
        ...
        if (orphanedFiles.length > 10)
            console.log(`  ... y ${orphanedFiles.length - 10} más.`);
    }
    if (ghostUrls.length > 0) {
        ...
        if (ghostUrls.length > 10)
            console.log(`  ... y ${ghostUrls.length - 10} más.`);
    }
