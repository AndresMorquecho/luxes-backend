...
import { calcularMultaAtraso, normalizeHorariosLaborales, DEFAULT_HORARIOS_LABORALES, } from '../../shared/utils/horarioLaboralHelpers.js';
import { getOpcionesMarcacion, calcularHorasExtrasDesdeConfig, redondearAMediaHora, } from '../../features/asistencia/domain/marcacionLogic.js';
// ════════════════════════════════════════════════════════════════════════════
...
describe('calcularHorasExtrasDesdeConfig', () => {
    ...
    function ecUTC(dateStr, timeStr) {
    ...
    test('fin ANTES de 17:30 → lanza error', () => {
        ...
        assert.throws(() => calcularHorasExtrasDesdeConfig(diaConfig, finHora, DATE), { message: /posterior a la salida programada/i });
    });
...
describe('getOpcionesMarcacion — Medio Día', () => {
    ...
    test('Medio Día con SALIDA → día completado', () => {
        const opts = getOpcionesMarcacion([{ tipo: 'ENTRADA' }, { tipo: 'SALIDA' }], 'Medio Día', diaConfig);
        assert.equal(opts.length, 0);
    });
});
// ════════════════════════════════════════════════════════════════════════════
...
describe('expectedShiftHours según diaConfig', () => {
    ...
    function calcExpected(diaConfig, tipoContrato) {
...
describe('Cálculo de valor sugerido de horas extras', () => {
    function calcSugerido(horasExactas, valorMediaHora = 1.5) {
