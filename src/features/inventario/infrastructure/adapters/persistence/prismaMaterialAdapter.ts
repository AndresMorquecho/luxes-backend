...
export class PrismaMaterialAdapter implements MaterialRepositoryPort {
  ...
  async findAll(options?: {
    ...
    search?: string;
    categoria?: string;
    incluirDerivados?: boolean; // true = incluir rollos [R001],[R002] (para vista de inventario impresión)
  ...
  private buildPrestamosWhere(options: {
    estado?: string;
    fechaInicio?: string;
    fechaFin?: string;
    searchTool?: string;
    ...
    if (options.responsableId) where.responsableId = options.responsableId;
  ...
  async listPrestamosWithOptions(options?: {
    ...
    if (page && limit) {
      ...
      return { items: rows as unknown as PrestamoData[], total };
    }
