...
function toGastoDate(f: string | Date | undefined): Date {
...
export class NominaController {
  ...
  async deleteOvertime(req: Request, res: Response): Promise<Response> {
    try {
      const id = String(req.params.id);
  ...
  async getPayrolls(req: Request, res: Response): Promise<Response> {
    try {
      ...
      for (const emp of empleados) {
        ...
        const { diasAsistencia, diasFeriado, diasLaborados: originalDiasLaborados } = calcDiasLaborados(
          empAsistencias,
          feriados,
          fInicioStr,
          fFinStr,
        ...
        const existingEgresos = existing ? (typeof existing.egresos === 'string' ? JSON.parse(existing.egresos as string) : ((existing.egresos as any) || {})) : {};
        const permisosDetalleArr: any[] = Array.isArray(existingEgresos.permisosDetalle) ? existingEgresos.permisosDetalle : [];
        ...
        if (isFijo) {
          const dec4Val = (emp as any).decimoCuartoValor !== null && (emp as any).decimoCuartoValor !== undefined
            ? Number((emp as any).decimoCuartoValor)
            : 40.16;
          decimoCuartoQuincenal = Math.round((dec4Val / 2) * 100) / 100;

          const dec3Val = (emp as any).decimoTerceroValor !== null && (emp as any).decimoTerceroValor !== undefined
            ? Number((emp as any).decimoTerceroValor)
            : (sueldoMensual / 12);
        ...
        if (!existing) {
          ...
          if (existing.estado === "PENDIENTE" || existing.estado === "ABONO_PARCIAL") {
            const currentIngresos = typeof existing.ingresos === 'string' ? JSON.parse(existing.ingresos as string) : ((existing.ingresos as any) || {});
  ...
  async savePayroll(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const detalleList: any[] = Array.isArray(egresosFromBody.permisosDetalle) ? egresosFromBody.permisosDetalle : [];
