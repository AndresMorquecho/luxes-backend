...
import {
  ...
  updateCategoryImage,
} from '../persistence/prismaLandingAdapter.js';
...
const sanitizeItemId = (itemId: string) => itemId.replace(/[^a-zA-Z0-9-_]/g, '');

// ─── Helpers ──────────────────────────────────────────────────────────────────

const parseTagsField = (tags: string): string[] => {
  try {
...
export class LandingController {
  ...
  async uploadImage(req: Request, res: Response) {
    try {
      const { section, itemId } = req.body as { section?: string; itemId?: string };
      const file = req.file;
  ...
  async resetImage(req: Request, res: Response) {
    try {
  ...
  async getSocial(_req: Request, res: Response) {
    try {
  ...
  async updateSocial(req: Request, res: Response) {
    try {
      const { facebook, instagram, tiktok } = req.body as {
        facebook?: string;
        instagram?: string;
        tiktok?: string;
      };
  ...
  async createCategory(req: Request, res: Response) {
    try {
      const { name, slug, order, active } = req.body as {
        name?: string;
        slug?: string;
        order?: number;
        active?: boolean;
  ...
  async addCategoryImage(req: Request, res: Response) {
    try {
      const id = String(req.params.id);
      const file = req.file;
      ...
      if ((error as Error).message === 'MAX_IMAGES_REACHED') {
  ...
  async updateCategoryImage(req: Request, res: Response) {
    try {
