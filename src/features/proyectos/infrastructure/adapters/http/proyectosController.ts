...
import { safeRemoveDir, safeUnlinkFile, safeResolvePath } from '../../../../../shared/utils/pathSafetyHelper.js';
...
async function getPersonalEncuesta(
  proyectoId: string,
  datosInstalacion: Record<string, any>,
  instalacion: {
  ...
  const personalMap = new Map<
    string,
    { empleadoId: string; id: string; nombre: string; roles: string[] }
  >();

  const addPersona = (idStr: string, nombreRaw: string, rolStr: string) => {
  ...
  if (Array.isArray(desdeDatos) && desdeDatos.length > 0) {
    desdeDatos.forEach((p: Record<string, any>, index: number) => {
      if (p.nombre) {
        addPersona(
          String(p.empleadoId || p.id || `personal-${index}`),
          String(p.nombre),
          String(p.rol || 'Instalador Principal'),
        );
      }
    });
    ...
    instalacion.personalAsignado.forEach((p: any, index: number) => {
      if (p.empleado?.nombre) {
        addPersona(
          p.empleadoId || `personal-${index}`,
          p.empleado.nombre,
          p.rol || 'Instalador Principal',
        );
      }
    });
  }
...
export class ProyectosController {
  ...
  async avanzarFase(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const targetIdx = FASES_ORDEN.indexOf(String(fase) as (typeof FASES_ORDEN)[number]);
      if (targetIdx > 0) {
      ...
      if (seCompletaInstalacion) {
        ...
        try {
          await notificarDevolucionHerramientasInstalacion({
            ...
            datosInstalacion: datosAGuardar as Record<string, unknown>,
          });
        } catch (devolError) {
          console.error('[Proyecto] Error registrando devoluciones de herramientas:', devolError);
        }
      }
  ...
  async sincronizarDevoluciones(_req: Request, res: Response): Promise<Response> {
    try {
      const procesados = await sincronizarDevolucionesInstalacionesPendientes();
      return res.status(200).json({ success: true, data: { procesados } });
    } catch (error) {
      console.error('[proyectos/sincronizarDevoluciones]', error);
      return res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: 'Error al sincronizar devoluciones' },
      });
    }
  }
  ...
  async serveArchivoProyecto(req: Request, res: Response): Promise<Response | void> {
    try {
  ...
  async deleteArchivoProyecto(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async uploadEvidenciaInstalacion(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      const file = req.file;
  ...
  async getProjectStats(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (hasDateFilter) completedWhere.fechaCompletado = dateFilter;
      ...
      if (hasDateFilter) activeWhere.fechaCreacion = dateFilter;
  ...
  async submitReclamo(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      ...
      await prisma.$executeRawUnsafe(
        `INSERT INTO reclamos_proyectos (id, proyecto_id, cliente_nombre, cliente_telefono, cliente_email, detalle, estado, fecha_creacion, fecha_actualizacion)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())`,
        reclamoId, proyecto.id, clienteNombre, clienteTelefono, clienteEmail, textoDetalle, 'PENDIENTE'
      );
  ...
  async listReclamos(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async updateEstadoReclamo(req: Request, res: Response): Promise<Response> {
    try {
      ...
      await prisma.$executeRawUnsafe(
        `UPDATE reclamos_proyectos SET estado = $1, notas_resolucion = COALESCE($2, notas_resolucion), fecha_actualizacion = NOW() WHERE id = $3`,
        estadoNormalizado, notas, String(reclamoId)
      );
   ...
   */
  async createBatchDiseno(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      const b = req.body || {};
   ...
   */
  async addArchivoToBatch(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!Array.isArray(batch.archivos)) batch.archivos = [];
      // Evitar duplicados por URL
      if (!batch.archivos.some((a: any) => a.url === archivo.url)) {
   ...
   */
  async enviarBatchImpresion(req: Request, res: Response): Promise<Response> {
    try {
      const { id, batchId } = req.params;
      const b = req.body || {};
