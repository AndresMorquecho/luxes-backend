  ...
  async registrarAsistencia(input: {
    ...
    if (proxima.tipo === 'ENTRADA') {
      ...
      if (lateHours > 0) {
        ...
        const hasAtrasoToday = permisosDetalle.some((p: { fecha: string; tipo: string }) => p.fecha === dateStr && p.tipo === 'ATRASO_QR');
        ...
        if (!hasAtrasoToday) {
          ...
          const updatedPermisos = [...permisosDetalle, newPermission];
          const newPermisoHoras = updatedPermisos.reduce((sum: number, item: { horas?: number }) => sum + Number(item.horas || 0), 0);
