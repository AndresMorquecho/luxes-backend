...
export class AsistenciaService {
  ...
  private static autoAsistenciaPromise: Promise<void> | null = null;

  async evaluarAutoAsistencias(): Promise<void> {
    ...
    AsistenciaService.autoAsistenciaPromise = (async () => {
      try {
        ...
        if (!empsAuto || empsAuto.length === 0) return;
        ...
        const parseMin = (timeStr?: string | null) => {
  ...
  async getAutoAsistenciaStatus(empleadoId: string): Promise<boolean> {
    try {
  ...
  async toggleAutoAsistenciaStatus(empleadoId: string, autoAsistencia: boolean): Promise<boolean> {
    try {
        ...
        if (eliminarMultaAsociada && targetFineType) {
          ...
          if (tipo === 'ENTRADA' && diaConfig?.entrada) {
            ...
            if (inicioAlm) {
              ...
              const tolerancia = Number((diaConfig as any)?.toleranciaAlmuerzo ?? 8);
              multaDolares = calcularMultaAtraso(atrasoMinutos, tolerancia);
              if (multaDolares > 0) {
          ...
          const existingIndex = permisosDetalle.findIndex((p: any) => p.fecha === dateStr && p.tipo === targetFineType);
