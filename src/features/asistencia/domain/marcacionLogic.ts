...
export type TipoMarcacion = (typeof SECUENCIA_MARCACIONES)[number]['tipo'] | 'FIN_HORAS_EXTRA' | 'SALIDA_PERMISO';
...
export function resolveProximaMarcacion(
  marks: MarcacionLike[], 
  tipoContrato: string = 'Tiempo Completo',
  diaConfig?: any
): ProximaMarcacionResult {
...
export function getOpcionesMarcacion(
  marks: MarcacionLike[], 
  tipoContrato: string = 'Tiempo Completo',
  diaConfig?: any
) {
  ...
  if (tipos.has('SALIDA_PERMISO')) return [] as typeof TIPOS_SELECCIONABLES[number][];
  ...
  if (!tipos.has('ENTRADA')) {
    ...
    if (currentMin > maxEntradaValida) {
      const parseTimeToMin = (timeStr?: string | null) => {
...
export function validateTipoMarcacion(
  marks: MarcacionLike[], 
  tipo: string, 
  tipoContrato: string = 'Tiempo Completo',
  diaConfig?: any
...
export function resolveTipoRegistro(
  marks: MarcacionLike[],
  options: { omitirAlmuerzo?: boolean; horaActual?: Date; tipo?: string; tipoContrato?: string; diaConfig?: any } = {},
): { tipo: string; label: string } {
...
export function puedeRegistrarMarcacion(
  marks: MarcacionLike[], 
  tipoContrato: string = 'Tiempo Completo',
  diaConfig?: any
): boolean {
 ...
 */
export function calcularHorasExtrasDesdeConfig(
  diaConfig: { salida: string },
  finHora: Date,
  dateStr: string,
): { horas: number; detalleHorario: string; salidaConfigAt: Date } {
 ...
 */
export function redondearAMediaHora(horas: number): number {
