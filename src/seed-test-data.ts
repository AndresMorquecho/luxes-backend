...
function addMin(timeStr: string, mins: number): string {
  const [h, m] = timeStr.split(':').map(Number);
  const total = h * 60 + m + mins;
  return `${pad(Math.floor(total / 60))}:${pad(total % 60)}`;
}

function ecToUTC(dateStr: string, timeStr: string): Date {
...
async function run() {
  ...
  const horarios = (config?.horariosLaborales as any) || {};
  const semana = horarios.semana || {};
  const entradaHora: string    = semana.entrada        || '08:00';
  const salidaHora: string     = semana.salida         || '17:30';
  const almInicioHora: string  = semana.inicioAlmuerzo || '13:00';
  const tolerancia: number     = Number(horarios.toleranciaMinutos ?? 8);
  ...
  if (!empFijo) throw new Error('No hay empleados fijos con sueldo');
  ...
  // Multas
  function calcMulta(mins: number, tol: number) {
    if (mins <= tol) return 0;
    const s = mins - tol; if (s <= 8) return 2; if (s <= 16) return 3; return 4;
  }
  ...
  if (nom) {
    ...
    const prevDet = Array.isArray(eg.permisosDetalle)
      ? eg.permisosDetalle.filter((p: any) => p.fecha !== HOY || !['ATRASO_QR','ATRASO_QR_ALMUERZO'].includes(p.tipo))
      : [];
    const newDet = [...prevDet, ...detalle];
    const newTotal = newDet.filter((r:any) => !r.eliminado).reduce((s:number,r:any) => s + (r.multaDolares ?? r.horas ?? 0), 0);
